...
function createLoopingAudio(url: string | null) {
  ...
  audio.addEventListener(
    'loadedmetadata',
    () => {
      if (!Number.isFinite(audio.duration) || audio.duration < 45) return;
      const seed = Array.from(url).reduce((total, character, index) => {
        return (total + character.charCodeAt(0) * (index + 1)) % 997;
      }, 0);
      const offsetRatio = 0.12 + (seed / 997) * 0.26;
      const maxOffset = Math.min(
        Math.max(audio.duration - 0.5, 0),
        Math.min(audio.duration * 0.38, 42),
      );
      const nextTime = Math.min(maxOffset, audio.duration * offsetRatio);
      if (nextTime > 0 && Number.isFinite(nextTime)) {
        try {
          audio.currentTime = nextTime;
        } catch {
          // Some browsers throw if the seek window is not ready yet.
        }
      }
    },
    { once: true },
...
async function playElement(audio: HTMLAudioElement | null) {
  if (!audio) return;
  try {
    await audio.play();
  } catch {
    // Browser policy or transient media issues should not break the shell.
  }
}
...
function setDeckVolumes(
  deck: Deck,
  primaryGain01: number,
  secondaryGain01: number,
) {
  if (deck.primary) deck.primary.volume = clampGain(primaryGain01);
  if (deck.secondary) deck.secondary.volume = clampGain(secondaryGain01);
}

function pauseDeck(deck: Deck) {
  stopElement(deck.primary);
  stopElement(deck.secondary);
}
...
export function GlobalSurfaceAtmosphereRuntime() {
  ...
  const deckVariants = useMemo(
    () => buildSurfaceAtmosphereDeckVariants(state),
    [state],
  );
  const deckSpec =
    deckVariants[sceneVariantIndex % deckVariants.length] ?? deckVariants[0];
  ...
  }, [surfaceKey, state.sceneId, state.variant]);
  ...
  useEffect(() => {
    ...
    const dwellDurationMs =
      settings.intensity === 'immersive' ? 150_000 : 210_000;
  ...
  useEffect(() => {
    ...
    const unlock = () => {
      ...
      void playElement(activeDeck.primary);
      void playElement(activeDeck.secondary);
    };
  ...
  useEffect(() => {
    ...
    if (unlockedRef.current) {
      void playElement(nextDeck.primary);
      void playElement(nextDeck.secondary);
    }
