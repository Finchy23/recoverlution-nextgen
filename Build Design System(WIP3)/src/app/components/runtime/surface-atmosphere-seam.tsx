...
const STORAGE_KEY = 'recoverlution_wip3_surface_atmosphere_settings_v1';
...
function readInitialSettings(): SurfaceAtmosphereSettings {
  ...
  try {
    ...
    return {
      ...
      adaptToSurface:
        parsed.adaptToSurface ??
        DEFAULT_SURFACE_ATMOSPHERE_SETTINGS.adaptToSurface,
      rememberPreference:
        parsed.rememberPreference ??
        DEFAULT_SURFACE_ATMOSPHERE_SETTINGS.rememberPreference,
...
export function SurfaceAtmosphereProvider({
  children,
}: {
  children: ReactNode;
}) {
  const [settings, setSettings] = useState<SurfaceAtmosphereSettings>(
    readInitialSettings,
  );
  ...
  useEffect(() => {
    ...
    try {
      ...
      // Best-effort persistence for the testing seam.
    }
  }, [settings]);

  const updateSettings = useCallback(
    (next: Partial<SurfaceAtmosphereSettings>) => {
      setSettings((current) => ({
        ...current,
        ...next,
      }));
