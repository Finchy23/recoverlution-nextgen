/**
 * ATOM: STILL-POINT — The Paradox of Arrival
 * Approach: Install Schema — "You're already here."
 *
 ...
 *
 * The system teaches: disruption is temporary. The center holds.
 *
...
const SETTLE_RATE = 0.9978;        // How quickly orbits tighten (per frame) — resolves ~12s
const SCATTER_FORCE = 8;           // Force of touch disruption
const SCATTER_RECOVERY = 0.97;     // How quickly particles recover from scatter
const CENTER_GLOW_BASE = 0.04;
const CENTER_GLOW_BREATH = 0.03;
const RESOLVE_THRESHOLD = 0.15;    // Avg orbit radius fraction for resolve
  ...
  useEffect(() => {
    ...
    const render = () => {
      ...
      for (const pt of s.particles) {
        // Settle: orbits slowly tighten — state change, always runs
        ...
        // Shimmer
        const shimmer = p.reducedMotion ? 0.8 : 0.6 + 0.4 * Math.sin(s.frameCount * 0.015 + pt.phase);

        // Distance-based alpha (closer to center = brighter)
