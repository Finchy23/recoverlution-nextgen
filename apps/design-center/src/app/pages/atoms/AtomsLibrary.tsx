    ...
    <div style={{ position: 'absolute', inset: 0, overflow: 'hidden' }}>
        ...
        {AtomComponent ? (
          ...
          >
            ...
            <div
              style={{
                width:
                  atomsLayout.blueprintRingBase + amplitude * atomsLayout.blueprintRingBreathRange,
                height:
                  atomsLayout.blueprintRingBase + amplitude * atomsLayout.blueprintRingBreathRange,
                borderRadius: '50%',
            ...
            >
              <div
                style={{
                  width:
                    atomsLayout.blueprintDotBase + amplitude * atomsLayout.blueprintDotBreathRange,
                  height:
                    atomsLayout.blueprintDotBase + amplitude * atomsLayout.blueprintDotBreathRange,
                  borderRadius: '50%',
      ...
      <div
        style={{
          ...
          inset: 0,
          background:
            'linear-gradient(to bottom, rgba(0,0,0,0.15) 0%, transparent 20%, transparent 80%, rgba(0,0,0,0.1) 100%)',
          pointerEvents: 'none',
          zIndex: 6,
        }}
      />
      ...
      <div>
        ...
        <AnimatePresence mode="wait">
          ...
          >
            ...
            >
              {meta.intent.length > 200 ? meta.intent.slice(0, 200) + '…' : meta.intent}
            </div>
          </motion.div>
        </AnimatePresence>
      </div>
            ...
            >
              <div
                style={{
                  ...
                  borderRadius: '50%',
                  background:
                    state.atmosphereId === atm.id ? colors.neutral.white : surfaces.glass.border,
          ...
          {BACKGROUND_ENGINES.map((eng) => (
            <button
              ...
              style={{
                ...
                cursor: 'pointer',
                background:
                  state.backgroundEngineId === eng.id ? surfaces.glass.light : 'rgba(0,0,0,0)',
                textAlign: 'left',
                transition: 'background 0.2s',
              }}
            >
              <div
                style={{
                  ...
                  borderRadius: '50%',
                  background:
                    state.backgroundEngineId === eng.id
  ...
  return (
    <div>
      <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 4 }}>
        ...
        <div
          style={{ fontFamily: fonts.mono, fontSize: 9, color: colors.neutral.white, opacity: 0.3 }}
        >
          {display}
        </div>
      </div>
...
export default function AtomsLibrary() {
  ...
  const [state, setState] = useState<CompositionState>(() => {
    ...
    if (paramAtom && ATOM_COMPONENTS[paramAtom]) {
      return { ...INITIAL_STATE, atomId: paramAtom };
    }
  ...
  const atomParam = searchParams.get('atom') as AtomId | null;

  const { setContent } = useDeviceMirror();

  useEffect(() => {
    if (atomParam && atomParam !== state.atomId && ATOM_COMPONENTS[atomParam]) {
      setState((s) => ({ ...s, atomId: atomParam }));
    }
  }, [atomParam, state.atomId]);

  useEffect(() => {
    if (atomParam === state.atomId) return;
    const next = new URLSearchParams(searchParams);
    next.set('atom', state.atomId);
    setSearchParams(next, { replace: true });
  }, [state.atomId, atomParam, searchParams, setSearchParams]);
