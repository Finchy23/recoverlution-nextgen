...
export default function TrimTabCorrectionAtom({
  ...
  useEffect(() => {
    ...
    const render = () => {
      ...
      // ── Field-scale course correction ────────────────────
      if (s.trimTabFlipped) {
        const startX = w * 0.14;
        const startY = cy + px(0.045, minDim);
        const badCtrlX = w * 0.52;
        const badCtrlY = cy + px(0.13, minDim);
        const goodCtrlX = w * 0.56;
        const goodCtrlY = cy - px(0.015, minDim) * s.correctionProgress;
        const endX = w * 0.88;
        const badEndY = cy + px(0.09, minDim);
        const goodEndY = cy - px(0.01, minDim) * s.correctionProgress;

        ctx.beginPath();
        ctx.moveTo(startX, startY);
        ctx.quadraticCurveTo(badCtrlX, badCtrlY, endX, badEndY);
        ctx.strokeStyle = rgba(s.accentRgb, ALPHA.content.max * 0.05 * entrance * (1 - s.correctionProgress));
        ctx.lineWidth = px(0.0012, minDim);
        ctx.setLineDash([px(0.006, minDim), px(0.006, minDim)]);
        ctx.stroke();
        ctx.setLineDash([]);

        ctx.beginPath();
        ctx.moveTo(startX, startY);
        ctx.quadraticCurveTo(goodCtrlX, goodCtrlY, endX, goodEndY);
        ctx.strokeStyle = rgba(s.primaryRgb, ALPHA.background.max * 0.16 * entrance * s.correctionProgress);
        ...
        ctx.moveTo(startX, startY);
        ctx.quadraticCurveTo(goodCtrlX, goodCtrlY, endX, goodEndY);
        ctx.strokeStyle = rgba(s.primaryRgb, ALPHA.glow.max * 0.22 * entrance * s.correctionProgress);
