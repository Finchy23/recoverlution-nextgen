...
export default function ApexEconomyAtom({
  ...
  useEffect(() => {
    ...
    const render = () => {
      ...
      // ── 6. Seal ring ──────────────────────────────────────
      if (s.partialClear && !s.sealed) {
        const fractureR = px(SEAL_R * 0.78, minDim);
        for (let i = 0; i < 3; i++) {
          const start = -Math.PI / 2 + i * (Math.PI * 0.72);
          const end = start + Math.PI * 0.36;
          ctx.beginPath();
          ctx.arc(cx, cy, fractureR + i * px(0.01, minDim), start, end);
          ctx.strokeStyle = rgba(s.accentRgb, ALPHA.content.max * 0.08 * entrance * (0.85 - s.tension * 0.25));
