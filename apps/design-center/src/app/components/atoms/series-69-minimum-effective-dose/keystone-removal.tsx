...
export default function KeystoneRemovalAtom({
  ...
  useEffect(() => {
    ...
    const render = () => {
      ...
      if (s.keystoneRemoved) {
        ctx.beginPath();
        const archTopY = h * (ARCH_CENTER_Y - ARCH_RADIUS);
        const archRadiusPx = px(ARCH_RADIUS, minDim);
        ctx.arc(cx, archTopY + archRadiusPx, archRadiusPx, Math.PI, 0);
        ctx.strokeStyle = rgba(s.primaryRgb, ALPHA.content.max * 0.08 * entrance * (s.completed ? 1 : 0.6));
