  ...
  useEffect(() => {
    ...
    const render = () => {
      ...
      if (s.completed) {
        ...
        ctx.beginPath();
        ctx.arc(cx, cy, cR * 1.45, 0, Math.PI * 2);
        ctx.strokeStyle = rgba(s.primaryRgb, ALPHA.content.max * 0.08 * entrance);
        ctx.lineWidth = px(0.0018, minDim);
        ctx.stroke();

        for (let i = 0; i < 8; i++) {
          const a = (Math.PI * 2 * i) / 8 + s.frameCount * 0.01;
          const px2 = cx + Math.cos(a) * cR * 1.95;
          const py = cy + Math.sin(a) * cR * 1.95;
          ctx.beginPath();
          ctx.arc(px2, py, px(0.0024, minDim), 0, Math.PI * 2);
