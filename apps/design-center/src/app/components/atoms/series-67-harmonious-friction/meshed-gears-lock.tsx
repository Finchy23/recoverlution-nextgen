  ...
  useEffect(() => {
    ...
    const render = () => {
      ...
      if (s.locked) {
        ...
        const corridorStartX = ax - gearR * 0.4;
        const corridorEndX = bx + gearR * 0.4;
        for (let i = -1; i <= 1; i++) {
          const y = midY + i * px(0.024, minDim);
          ctx.beginPath();
          ctx.moveTo(corridorStartX, y);
          ctx.lineTo(corridorEndX, y);
          ctx.strokeStyle = rgba(s.primaryRgb, ALPHA.background.max * 0.14 * entrance);
          ctx.lineWidth = px(0.008, minDim);
          ctx.lineCap = 'round';
          ctx.stroke();

          for (let pIdx = 0; pIdx < 4; pIdx++) {
            const t = ((s.frameCount * 0.015) + pIdx * 0.23) % 1;
            const px2 = corridorStartX + (corridorEndX - corridorStartX) * t;
            ctx.beginPath();
            ctx.arc(px2, y, px(PARTICLE_SIZE.sm, minDim), 0, Math.PI * 2);
            ctx.fillStyle = rgba(s.accentRgb, ALPHA.content.max * 0.16 * entrance);
            ctx.fill();
          }
        }
      }
