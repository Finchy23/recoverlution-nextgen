...
export default function KineticReturnCurveAtom({
  ...
  useEffect(() => {
    ...
    const render = () => {
      ...
      const coreY = h * 0.78;
      s.sourceCollapse = Math.max(0, s.sourceCollapse - 0.02 * ms);
      ...
      if (s.sourceCollapse > 0.001) {
        const apertureR = px(0.015 + s.sourceCollapse * 0.055, minDim);
        const sourceX = s.projX * w;
        const sourceY = h * 0.04;
        ctx.beginPath();
        ctx.arc(sourceX, sourceY, apertureR, 0, Math.PI * 2);
        ctx.strokeStyle = rgba(s.primaryRgb, ALPHA.content.max * 0.55 * s.sourceCollapse * entrance);
        ctx.lineWidth = px(0.002, minDim);
        ctx.stroke();

        ctx.beginPath();
        ctx.moveTo(sourceX - apertureR * 0.8, sourceY);
        ctx.lineTo(sourceX + apertureR * 0.8, sourceY);
        ctx.moveTo(sourceX, sourceY - apertureR * 0.8);
        ctx.lineTo(sourceX, sourceY + apertureR * 0.8);
        ctx.strokeStyle = rgba(s.primaryRgb, ALPHA.atmosphere.max * 0.3 * s.sourceCollapse * entrance);
