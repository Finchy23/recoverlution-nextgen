...
export default function EscalationCoolAtom({
  ...
  useEffect(() => {
    ...
    const render = () => {
      ...
      const coolProgress = 1 - s.heat;

      if (coolProgress > 0.02 || s.completed) {
        const fieldAlpha = ALPHA.background.min * (1.4 + coolProgress * 2.6) * entrance;
        const fieldGrad = ctx.createRadialGradient(drawX, drawY, 0, drawX, drawY, minDim * (0.12 + coolProgress * 0.42));
        fieldGrad.addColorStop(0, rgba(s.primaryRgb, fieldAlpha));
        fieldGrad.addColorStop(0.55, rgba(s.primaryRgb, fieldAlpha * 0.35));
        fieldGrad.addColorStop(1, rgba(s.primaryRgb, 0));
        ctx.fillStyle = fieldGrad;
        ctx.fillRect(0, 0, w, h);

        for (let i = 0; i < 4; i++) {
          const waveY = drawY + (i - 1.5) * minDim * 0.09;
          ctx.beginPath();
          for (let step = 0; step <= 20; step++) {
            const waveX = (step / 20) * w;
            const offset = Math.sin(step * 0.65 + s.frameCount * 0.03 * ms + i) * px(0.003, minDim) * s.heat;
            if (step === 0) ctx.moveTo(waveX, waveY + offset);
            else ctx.lineTo(waveX, waveY + offset);
          }
          ctx.strokeStyle = rgba(s.primaryRgb, ALPHA.atmosphere.min * coolProgress * 1.5 * entrance);
