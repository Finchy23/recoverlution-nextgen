...
export default function ApexWuWeiAtom({
  ...
  useEffect(() => {
    ...
    const render = () => {
      ...
      // When surrender lands, the center should read as an untouched sovereign moat,
      // not just a brighter node. The world parts around it.
      if (s.surrendered || s.sealed) {
        const moatProgress = s.sealed ? 1 : Math.max(0.2, s.surrenderProgress);
        const moatR = coreR * (5.5 + moatProgress * 5.5);
        const moatBand = coreR * (1.1 + moatProgress * 0.6);

        ctx.beginPath();
        ctx.arc(cx, cy, moatR, 0, Math.PI * 2);
        ctx.strokeStyle = rgba(s.primaryRgb, ALPHA.content.max * 0.05 * moatProgress * entrance);
        ctx.lineWidth = moatBand;
        ctx.stroke();

        ctx.beginPath();
        ctx.arc(cx, cy, moatR, 0, Math.PI * 2);
        ctx.strokeStyle = rgba(
          lerpColor(s.primaryRgb, [255, 255, 255] as RGB, 0.16),
          ALPHA.content.max * 0.11 * moatProgress * entrance,
        );
        ...
        for (let i = 0; i < 4; i++) {
          ...
          const bx = cx + Math.cos(angle) * moatR;
          const by = cy + Math.sin(angle) * moatR;
          const beaconR = coreR * (0.34 + moatProgress * 0.14);

          const beaconGlow = ctx.createRadialGradient(bx, by, 0, bx, by, beaconR * 5);
          beaconGlow.addColorStop(0, rgba(s.primaryRgb, ALPHA.glow.max * 0.12 * moatProgress * entrance));
          beaconGlow.addColorStop(0.35, rgba(s.primaryRgb, ALPHA.glow.min * 0.08 * moatProgress * entrance));
          beaconGlow.addColorStop(1, rgba(s.primaryRgb, 0));
          ctx.fillStyle = beaconGlow;
          ctx.beginPath();
          ctx.arc(bx, by, beaconR * 5, 0, Math.PI * 2);
          ctx.fill();

          ctx.beginPath();
          ctx.arc(bx, by, beaconR, 0, Math.PI * 2);
          ctx.fillStyle = rgba([255, 255, 255] as RGB, ALPHA.content.max * 0.22 * moatProgress * entrance);
          ctx.fill();
        }
      }
