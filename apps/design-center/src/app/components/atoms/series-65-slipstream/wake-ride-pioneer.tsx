    ...
    const render = () => {
      ...
      if (!p.reducedMotion && !s.completed) {
        ...
        if (s.inWake) {
          ...
          s.cruiseTime++;
          s.wakeField = Math.min(1, s.wakeField + 0.02);
      ...
      if (s.pioneerX > 0) {
        ...
        const wakeStrength = s.completed ? 1 : s.wakeField;
        if (wakeStrength > 0.01) {
          const focusX = chStartX + (chEndX - chStartX) * 0.42;
          const focusY = (chTopY + chBotY) / 2;
          for (let i = 0; i < 4; i++) {
            const frac = i / 3;
            const ringW = (chEndX - chStartX) * (0.18 + frac * 0.16);
            const ringH = chH * (0.22 + frac * 0.16);
            ctx.beginPath();
            ctx.ellipse(focusX + frac * px(0.018, minDim), focusY, ringW, ringH, 0, 0, Math.PI * 2);
            ctx.strokeStyle = rgba(s.primaryRgb, (0.05 + frac * 0.04) * wakeStrength * entrance);
            ctx.lineWidth = px(STROKE.hairline, minDim);
            ctx.stroke();
          }

          for (let i = 0; i < 3; i++) {
            const bandY = chTopY + (i + 1) * (chH / 4);
            ctx.beginPath();
            ctx.moveTo(chStartX - px(0.012, minDim), bandY);
            ctx.quadraticCurveTo(
              focusX,
              bandY + (i - 1) * px(0.012, minDim),
              Math.min(w * 1.02, chEndX + px(0.08, minDim)),
              bandY
            );
            ctx.strokeStyle = rgba(s.primaryRgb, 0.08 * wakeStrength * entrance * ms);
            ctx.lineWidth = px(STROKE.hairline, minDim);
            ctx.stroke();
          }
        }
      ...
      if (s.completed && p.phase !== 'resolve') {
        ...
        if (s.respawnTimer <= 0) {
          ...
          s.completed = false; s.wakeField = 0;
        }
      }
