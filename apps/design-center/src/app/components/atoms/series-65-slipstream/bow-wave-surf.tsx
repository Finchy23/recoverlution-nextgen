...
function freshState(color: string, accent: string): WaveState {
  return {
    ...
    surfTime: 0, completed: false, respawnTimer: 0, spray: [], surfTunnel: 0,
  };
}
    ...
    const render = () => {
      ...
      if (!p.reducedMotion && !s.completed) {
        ...
        if (s.locked) {
          ...
          s.surfTime++;
          s.surfTunnel = Math.min(1, s.surfTunnel + 0.02);
      ...
      const tunnelStrength = s.completed ? 1 : s.surfTunnel;
      if (tunnelStrength > 0.01) {
        const tunnelCx = waveLeadX + w * 0.085;
        const tunnelCy = waveTopY + px(0.06, minDim);
        for (let i = 0; i < 4; i++) {
          const frac = i / 3;
          const arcW = px(0.09 + frac * 0.045, minDim) * (1 + tunnelStrength * 0.7);
          const arcH = px(0.055 + frac * 0.03, minDim) * (1 + tunnelStrength * 0.4);
          ctx.beginPath();
          ctx.ellipse(tunnelCx + frac * px(0.015, minDim), tunnelCy, arcW, arcH, -0.3, Math.PI * 1.08, Math.PI * 1.95);
          ctx.strokeStyle = rgba(s.primaryRgb, (0.06 + frac * 0.04) * tunnelStrength * entrance);
          ctx.lineWidth = px(STROKE.hairline, minDim);
          ctx.stroke();
        }

        for (let i = 0; i < 3; i++) {
          const corridorY = tunnelCy + (i - 1) * px(0.018, minDim);
          ctx.beginPath();
          ctx.moveTo(tunnelCx - px(0.015, minDim), corridorY);
          ctx.quadraticCurveTo(
            tunnelCx + px(0.07, minDim),
            corridorY + px(0.01, minDim),
            Math.min(w * 1.02, tunnelCx + px(0.22, minDim)),
            corridorY + px(0.006, minDim)
          );
          ctx.strokeStyle = rgba(s.primaryRgb, 0.08 * tunnelStrength * entrance * ms);
          ctx.lineWidth = px(STROKE.hairline, minDim);
          ctx.stroke();
        }
      }
      ...
      if (s.completed && p.phase !== 'resolve') {
        ...
        if (s.respawnTimer <= 0) {
          ...
          s.surfTime = 0; s.completed = false; s.spray = []; s.surfTunnel = 0;
        }
      }
