  ...
  useEffect(() => {
    ...
    const render = () => {
      ...
      if (interp > 0.3) {
        ...
        if (interp > 0.58) {
          const laneAlpha = Math.min(1, (interp - 0.58) / 0.32) * entrance;
          const offsets = [-0.018, 0.018];

          for (const offset of offsets) {
            ctx.beginPath();
            for (let i = 0; i < s.points.length; i++) {
              const px_ = s.points[i].cx * w;
              const py_ = s.points[i].cy * h + offset * minDim;
              if (i === 0) ctx.moveTo(px_, py_);
              else {
                const prev = s.points[i - 1];
                const cpx = (prev.cx * w + px_) / 2;
                const cpy = (prev.cy * h + py_) / 2;
                ctx.quadraticCurveTo(prev.cx * w, prev.cy * h + offset * minDim, cpx, cpy);
              }
            }
            ctx.strokeStyle = rgba(s.primaryRgb, ALPHA.glow.max * 0.08 * laneAlpha);
          ...
          for (let i = 4; i < s.points.length; i += 9) {
            const px_ = s.points[i].cx * w;
            const py_ = s.points[i].cy * h;
            ctx.beginPath();
            ctx.arc(px_, py_, px(PARTICLE_SIZE.dot * 1.2, minDim), 0, Math.PI * 2);
            ctx.fillStyle = rgba([255, 255, 255] as RGB, ALPHA.content.max * 0.14 * laneAlpha);
            ctx.fill();
          }
        }
