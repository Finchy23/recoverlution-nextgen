...
function freshState(c: string, a: string): ApexState {
  return {
    ...
    completed: false, respawnTimer: 0, sovereignField: 0,
  };
}
  ...
  useEffect(() => {
    ...
    const render = () => {
      ...
      if (!p.reducedMotion && !s.completed) {
        ...
        s.sovereignField = Math.min(1, 1 - aliveCount / CHAOS_COUNT);
      ...
      const sovereignStrength = s.completed ? 1 : s.sovereignField;
      if (sovereignStrength > 0.01) {
        const recomposedR = px(Math.max(s.sphereR, 0.12 + sovereignStrength * 0.12), minDim);
        for (let i = 0; i < 3; i++) {
          const frac = i / 2;
          ctx.beginPath();
          ctx.ellipse(cx, cy, recomposedR * (0.75 + frac * 0.22), recomposedR * (0.42 + frac * 0.18), 0, 0, Math.PI * 2);
          ctx.strokeStyle = rgba(white, (0.05 + frac * 0.04) * sovereignStrength * entrance);
          ctx.lineWidth = px(STROKE.hairline, minDim);
          ctx.stroke();
        }

        for (let i = 0; i < 6; i++) {
          const axisA = s.spinAngle * 0.18 + (i / 6) * Math.PI * 2;
          ctx.beginPath();
          ctx.moveTo(cx + Math.cos(axisA) * coreR * 1.4, cy + Math.sin(axisA) * coreR * 1.4);
          ctx.lineTo(cx + Math.cos(axisA) * recomposedR * 0.95, cy + Math.sin(axisA) * recomposedR * 0.95);
          ctx.strokeStyle = rgba(white, 0.05 * sovereignStrength * entrance * ms);
          ctx.lineWidth = px(STROKE.hairline, minDim);
          ctx.stroke();
        }
      }
      ...
      if (s.completed && p.phase !== 'resolve') {
        ...
        if (s.respawnTimer <= 0) {
          ...
          s.dust = []; s.completed = false; s.sovereignField = 0;
        }
      }
