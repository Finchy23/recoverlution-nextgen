...
function freshState(c: string, a: string): DeflectState {
  return {
    ...
    completed: false, respawnTimer: 0, haloStrength: 0,
  };
}
...
export default function OrbitalDeflectionAtom({
  ...
  useEffect(() => {
    ...
    const render = () => {
        ...
        for (const proj of s.projectiles) {
          ...
          if (dist < FIELD_RADIUS && s.rpm > 0.1) {
            ...
            if (!proj.deflected) {
              proj.deflected = true;
              s.deflections++;
              s.haloStrength = Math.min(1, s.haloStrength + 0.12);
      ...
      const haloStrength = s.completed ? 1 : s.haloStrength;
      if (haloStrength > 0.01) {
        const haloR = fieldR * (1.1 + haloStrength * 0.18);
        for (let i = 0; i < 3; i++) {
          const offset = i * 0.28;
          ctx.beginPath();
          ctx.arc(cx, cy, haloR + i * px(0.014, minDim), s.fieldAngle + offset, s.fieldAngle + offset + Math.PI * 0.55);
          ctx.strokeStyle = rgba(s.primaryRgb, (0.06 + i * 0.03) * haloStrength * entrance);
          ctx.lineWidth = px(STROKE.hairline, minDim);
          ctx.stroke();
        }

        for (let i = 0; i < 4; i++) {
          const gateA = s.fieldAngle + i * Math.PI / 2;
          const gateR = haloR + px(0.018, minDim);
          const gx = cx + Math.cos(gateA) * gateR;
          const gy = cy + Math.sin(gateA) * gateR;
          ctx.beginPath();
          ctx.arc(gx, gy, px(0.01, minDim), 0, Math.PI * 2);
          ctx.fillStyle = rgba(s.primaryRgb, 0.09 * haloStrength * entrance * ms);
          ctx.fill();
        }
      }
      ...
      if (s.completed && p.phase !== 'resolve') {
        ...
        if (s.respawnTimer <= 0) {
          ...
          s.spawnTimer = 30; s.completed = false; s.haloStrength = 0;
        }
      }
