...
export default function TangledThreadWeaveAtom({
  color,
  accentColor,
  viewport,
  phase,
  ...
  useEffect(() => {
    ...
    const render = () => {
      ...
      if (p.phase === 'resolve') s.altitude += (1 - s.altitude) * 0.08;
      s.drift += 0.012;

      const reveal = easeOutCubic(clamp(s.altitude, 0, 1));
      ...
      const thread = lerpColor(accent, [255, 214, 154], 0.5);
      const portrait = lerpColor(primary, [244, 247, 255], 0.88);
      ctx.fillStyle = rgba(deep, 0.96 * entrance);
      ctx.fillRect(0, 0, w, h);

      for (let i = 0; i < 18; i += 1) {
        const x = w * (0.18 + (i % 6) * 0.12);
        ctx.beginPath();
        ctx.moveTo(x, h * 0.22);
        for (let j = 0; j < 5; j += 1) {
          const px = x + Math.sin(s.drift * 1.2 + i * 0.8 + j) * minDim * 0.12 * (1 - reveal);
          const py = h * (0.22 + j * 0.14);
          ctx.lineTo(px, py);
        }
        ctx.strokeStyle = rgba(thread, (0.08 + (1 - reveal) * 0.18) * entrance);
      ...
      if (reveal > 0.08) {
        ctx.beginPath();
        ctx.moveTo(cx - minDim * 0.18, cy - minDim * 0.14);
        ctx.quadraticCurveTo(cx, cy - minDim * 0.28, cx + minDim * 0.18, cy - minDim * 0.14);
        ctx.lineTo(cx + minDim * 0.12, cy + minDim * 0.18);
        ctx.lineTo(cx - minDim * 0.12, cy + minDim * 0.18);
        ctx.closePath();
        ctx.strokeStyle = rgba(portrait, reveal * 0.86 * entrance);
    ...
    const onMove = (e: PointerEvent) => {
      ...
      if (dy > 0) {
        stateRef.current.altitude = clamp(stateRef.current.altitude + (dy / viewport.height) * 1.9, 0, 1);
      }
