...
export default function ViscoelasticCreepAtom({
  ...
  useEffect(() => {
    ...
    const render = () => {
      ...
      if (s.deform > 0.42) {
        const shelfAlpha = ((s.deform - 0.42) / 0.58) * entrance;
        const shelfY = cy + radY * 0.9;
        const shelfW = radX * (1.25 + s.deform * 0.5);
        const shelfH = radY * 0.55;

        const shelfGrad = ctx.createLinearGradient(cx - shelfW / 2, shelfY, cx + shelfW / 2, shelfY);
        shelfGrad.addColorStop(0, rgba(s.primaryRgb, 0));
        shelfGrad.addColorStop(0.2, rgba(s.primaryRgb, ALPHA.glow.min * 0.2 * shelfAlpha));
        shelfGrad.addColorStop(0.5, rgba(s.primaryRgb, ALPHA.glow.max * 0.18 * shelfAlpha));
        shelfGrad.addColorStop(0.8, rgba(s.primaryRgb, ALPHA.glow.min * 0.2 * shelfAlpha));
        shelfGrad.addColorStop(1, rgba(s.primaryRgb, 0));
        ctx.fillStyle = shelfGrad;
        ctx.fillRect(cx - shelfW / 2, shelfY, shelfW, shelfH);

        for (let i = 0; i < 5; i++) {
          const tx = cx - shelfW * 0.4 + (i / 4) * shelfW * 0.8;
          ctx.beginPath();
          ctx.moveTo(tx, shelfY - radY * 0.18);
          ctx.lineTo(tx, shelfY + shelfH * 0.55);
          ctx.strokeStyle = rgba([255, 255, 255] as RGB, ALPHA.content.max * 0.08 * shelfAlpha);
