...
export default function ShapeMemoryRecallAtom({
  ...
  useEffect(() => {
    ...
    const render = () => {
      ...
      if (s.recall > 0.35) {
        const scaffoldAlpha = ((s.recall - 0.35) / 0.65) * entrance;
        const scaffoldR = geoR * (1.18 + s.recall * 0.08);

        ctx.beginPath();
        ctx.arc(cx, cy, scaffoldR, 0, Math.PI * 2);
        ctx.strokeStyle = rgba(s.primaryRgb, ALPHA.content.max * 0.08 * scaffoldAlpha);
        ctx.lineWidth = px(STROKE.hairline, minDim);
        ctx.stroke();

        for (let i = 0; i < 6; i++) {
          const angle = (i / 6) * Math.PI * 2;
          const ax = cx + Math.cos(angle) * scaffoldR;
          const ay = cy + Math.sin(angle) * scaffoldR;
          const tipX = cx + Math.cos(angle) * geoR * 0.82;
          const tipY = cy + Math.sin(angle) * geoR * 0.82;

          ctx.beginPath();
          ctx.moveTo(ax, ay);
          ctx.lineTo(tipX, tipY);
          ctx.strokeStyle = rgba(s.primaryRgb, ALPHA.atmosphere.max * 0.12 * scaffoldAlpha);
          ctx.lineWidth = px(STROKE.hairline, minDim);
          ctx.stroke();

          ctx.beginPath();
          ctx.arc(ax, ay, px(0.004, minDim), 0, Math.PI * 2);
          ctx.fillStyle = rgba([255, 255, 255] as RGB, ALPHA.content.max * 0.14 * scaffoldAlpha);
          ctx.fill();
        }
      }
