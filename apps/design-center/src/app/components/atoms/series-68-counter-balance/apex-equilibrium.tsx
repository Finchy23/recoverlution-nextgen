...
export default function ApexEquilibriumAtom({ breathAmplitude, reducedMotion, color, accentColor, viewport, phase, composed, onHaptic, onStateChange }: AtomProps) {
  ...
  useEffect(() => {
    ...
    const render = () => {
      ...
      if (s.survived > 0) {
        const eqAlpha = Math.min(1, s.survived / SURVIVE_BLOCKS) * entrance;
        const horizonY = fulcrumY - px(0.08, minDim);
        ctx.beginPath();
        ctx.moveTo(w * 0.2, horizonY);
        ctx.lineTo(w * 0.8, horizonY);
        ctx.strokeStyle = rgba(s.primaryRgb, ALPHA.background.max * 0.12 * eqAlpha);
        ...
        ctx.moveTo(w * 0.2, horizonY);
        ctx.lineTo(w * 0.8, horizonY);
        ctx.strokeStyle = rgba(s.primaryRgb, ALPHA.glow.max * 0.18 * eqAlpha);
        ctx.lineWidth = px(STROKE.light, minDim);
        ctx.stroke();

        for (let i = 0; i < 4; i++) {
          const bx = w * (0.28 + i * 0.14);
          ctx.beginPath();
          ctx.arc(bx, horizonY, px(0.0034, minDim), 0, Math.PI * 2);
          ctx.fillStyle = rgba(s.accentRgb, ALPHA.content.max * 0.12 * eqAlpha);
          ctx.fill();
        }
      }
