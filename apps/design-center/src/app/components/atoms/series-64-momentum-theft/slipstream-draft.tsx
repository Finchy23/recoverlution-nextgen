  ...
  useEffect(() => {
    ...
    const render = () => {
      ...
      if (s.inSlipstream || s.draftTime > 0) {
        ...
        const corridorStartX = Math.max(lx - leaderR * 6.2, w * 0.08);
        const corridorEndX = Math.min(lx + leaderR * 7.5, w * 0.94);
        const wakeHeight = minDim * 0.085;

        ctx.beginPath();
        ctx.moveTo(corridorStartX, ly - wakeHeight);
        ctx.lineTo(corridorEndX, ly - wakeHeight * 0.72);
        ctx.lineTo(corridorEndX, ly + wakeHeight * 0.72);
        ctx.lineTo(corridorStartX, ly + wakeHeight);
        ctx.closePath();
        ctx.fillStyle = rgba(s.primaryRgb, ALPHA.background.max * 0.14 * entrance);
        ctx.fill();

        ctx.beginPath();
        ctx.moveTo(corridorStartX, ly - wakeHeight * 0.55);
        ctx.lineTo(corridorEndX, ly - wakeHeight * 0.32);
        ...
        for (let i = 0; i < 6; i++) {
          const t = ((s.frameCount * 0.02) + i * 0.17) % 1;
          const x = corridorStartX + (corridorEndX - corridorStartX) * t;
          const y = ly + Math.sin(t * Math.PI * 4 + s.frameCount * 0.05) * wakeHeight * 0.1;
          ctx.beginPath();
