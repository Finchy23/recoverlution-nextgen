...
export default function AngularTheftStabilizeAtom({
  ...
  useEffect(() => {
    ...
    const render = () => {
      ...
      // ── Stabilized quiet field ────────────────────
      if (s.coreSpin > 0.25) {
        for (let i = 1; i <= 3; i++) {
          const haloR = coreR * (2.4 + i * 1.15);
          ctx.beginPath();
          ctx.arc(cx, cy, haloR, 0, Math.PI * 2);
          ctx.strokeStyle = rgba(s.primaryRgb, ALPHA.background.max * 0.08 * s.coreSpin * entrance * (1 - i * 0.18));
          ctx.lineWidth = px(0.0022, minDim);
          ctx.stroke();
        }

        const beaconR = coreR * 5.7;
        for (let i = 0; i < 4; i++) {
          const a = Math.PI / 4 + i * (Math.PI / 2);
          const bx = cx + Math.cos(a) * beaconR;
          const by = cy + Math.sin(a) * beaconR;
          ctx.beginPath();
          ctx.arc(bx, by, px(0.0042, minDim), 0, Math.PI * 2);
          ctx.fillStyle = rgba(s.accentRgb, ALPHA.content.max * 0.14 * s.coreSpin * entrance);
          ctx.fill();
        }
      }
