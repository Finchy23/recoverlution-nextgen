...
export default function TrebuchetCounterweightAtom({
  ...
  useEffect(() => {
    ...
    const render = () => {
      ...
      // ── Parabolic transfer arc ────────────────────
      if (s.launched || s.completed) {
        const startX = cx + leverLen * 0.4;
        const startY = leverY - userR - px(0.003, minDim);
        const peakX = cx + leverLen * 0.55;
        const peakY = leverY - minDim * 0.36;
        const endX = cx + leverLen * 0.78;
        const endY = leverY - minDim * 0.04;
        const arcAlpha = (s.completed ? 1 : Math.max(0.22, s.launchT)) * entrance;

        ctx.beginPath();
        ctx.moveTo(startX, startY);
        ctx.quadraticCurveTo(peakX, peakY, endX, endY);
        ctx.strokeStyle = rgba(s.primaryRgb, ALPHA.background.max * 0.18 * arcAlpha);
        ...
        ctx.moveTo(startX, startY);
        ctx.quadraticCurveTo(peakX, peakY, endX, endY);
        ctx.strokeStyle = rgba(s.primaryRgb, ALPHA.glow.max * 0.26 * arcAlpha);
        ctx.lineWidth = px(0.0022, minDim);
        ctx.stroke();

        ctx.beginPath();
        ctx.arc(endX, endY, px(0.026, minDim), 0, Math.PI * 2);
        ctx.strokeStyle = rgba(s.accentRgb, ALPHA.content.max * 0.16 * arcAlpha);
