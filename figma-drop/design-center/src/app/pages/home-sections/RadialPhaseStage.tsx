            ...
            <svg width="100%" height="100%" viewBox="0 0 300 150" preserveAspectRatio="xMidYMid meet" style={{ opacity: 0.7 }}>
              {[0, 1, 2, 3, 4, 5].map((i) => {
                ...
                return (
                  <motion.line
                    key={i}
                    x1={x} y1={75} x2={x + (i % 2 === 0 ? 15 : -15)} y2={75}
                    stroke={activePhase.color} strokeWidth="3" strokeLinecap="round"
                    initial={{ pathLength: 0, opacity: 0 }}
                    animate={{ pathLength: [0, 1, 1, 0], opacity: [0, 1, 1, 0], y1: 75, y2: [75, 40 + Math.random() * 70, 75] }}
  ...
  return (
    ...
    >
      <div className="absolute inset-0 w-full h-full">
        <div className="absolute inset-0" style={{ opacity: assetOpacity.section }}>
          <img src={assetUrl} alt="Phase Flow" loading="lazy" className="w-full h-full object-cover" style={{ opacity: assetOpacity.section }} />
        </div>
        <div className="absolute inset-0" style={{ background: reelOverlay.background }} />
      </div>

      <div className="relative w-full max-w-7xl mx-auto px-4 md:px-8" style={{ width: '100%', zIndex: 10 }}>
        <motion.div
                  ...
                  >
                    ...
                    <div style={{ fontSize: isMobile ? '16px' : '20px', color: activePhase.color, fontWeight: '500', letterSpacing: '-0.01em', lineHeight: '1.4', position: 'relative', paddingBottom: '4px', textAlign: 'center' }}>
                      ...
                      <motion.div
                        ...
                        style={{ position: 'absolute', bottom: 0, left: 0, right: 0, height: '2px', backgroundColor: activePhase.color, opacity: 0.4, transformOrigin: 'left' }}
                      />
                    </div>
                  </motion.div>
                )}
