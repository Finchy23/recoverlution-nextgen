...
// Motion/WAAPI animation errors that are non-fatal rendering glitches
const MOTION_WAAPI_PATTERNS = [
  'duration must be non-negative',
  "Failed to execute 'animate' on 'Element'",
] as const;
...
function isMotionWaapiError(event: ErrorEvent | PromiseRejectionEvent): boolean {
  const message =
    ('message' in event && typeof event.message === 'string' && event.message) ||
    ('reason' in event && event.reason?.message) ||
    ('reason' in event && String(event.reason)) ||
  ...
  if (typeof message === 'string') {
    return MOTION_WAAPI_PATTERNS.some(p => message.includes(p));
  }
  return false;
}
...
window.addEventListener('error', (event) => {
  ...
  if (isMotionWaapiError(event)) {
    event.preventDefault();
    if (import.meta.env.DEV) {
      console.warn('[ErrorSuppression] Motion WAAPI animation error suppressed:', event.message);
    }
    return;
  }
...
window.addEventListener('unhandledrejection', (event) => {
  ...
  if (isMotionWaapiError(event)) {
    event.preventDefault();
    if (import.meta.env.DEV) {
      console.warn('[ErrorSuppression] Motion WAAPI animation rejection suppressed');
    }
    return;
  }
  if (isViteTransientError(event)) {
...
console.log('%c[ErrorSuppression] Active — Figma + Vite + Motion WAAPI errors handled', 'color: #888');