...
// ── TALK: Save constellation names ──
app.post("/make-server-99d14421/talk/names", async (c) => {
  try {
    const body = await c.req.json();
    const { userId = 'anon', names } = body;

    if (!names || typeof names !== 'object') {
      return c.json({ error: 'Missing names object in constellation naming payload' }, 400);
    }

    const key = `talk:names:${userId}`;
    await kv.set(key, { userId, names, lastUpdated: Date.now() });

    console.log(`[TALK names] Stored ${key} — ${Object.keys(names).length} constellations named`);
    return c.json({ stored: true, count: Object.keys(names).length });
  } catch (err) {
    console.log(`[TALK names] Error storing names: ${err}`);
    return c.json({ error: `TALK constellation naming persistence failed: ${err}` }, 500);
  }
});

// ── TALK: Load constellation names ──
app.get("/make-server-99d14421/talk/names/:userId", async (c) => {
  try {
    const { userId } = c.req.param();
    const key = `talk:names:${userId}`;
    const data = await kv.get(key);
    ...
    if (!data) {
      return c.json({ names: {}, found: false });
    }

    console.log(`[TALK names] Loaded ${key} — ${Object.keys(data.names || {}).length} names`);
    return c.json({
      names: data.names || {},
      found: true,
    });
  } catch (err) {
    console.log(`[TALK names] Error loading names: ${err}`);
    return c.json({ error: `TALK constellation name load failed: ${err}` }, 500);
  }
});
...
// ═══════════════════════════════════════════════════
// TALK: Session Memory — Inter-session Continuity
// ═══════════════════════════════════════════════════
//
// Persists whisper history, territory depth, bridge crossings,
// and session count so the cosmos remembers you across visits.
//
// Key schema:
//   talk:memory:{userId}  — session memory envelope

// ── TALK: Save session memory ──
app.post("/make-server-99d14421/talk/memory", async (c) => {
  try {
    const body = await c.req.json();
    const { userId = 'anon', memory } = body;

    if (!memory) {
      return c.json({ error: 'Missing memory payload in TALK session memory save' }, 400);
    }

    const key = `talk:memory:${userId}`;
    await kv.set(key, JSON.stringify(memory));
    console.log(`[TALK memory] Session memory saved for ${userId}`);
    return c.json({ stored: true });
  } catch (err) {
    console.log(`[TALK memory] Error saving session memory: ${err}`);
    return c.json({ error: `TALK session memory save failed: ${err}` }, 500);
  }
});

// ── TALK: Load session memory ──
app.get("/make-server-99d14421/talk/memory/:userId", async (c) => {
  try {
    const { userId } = c.req.param();
    const key = `talk:memory:${userId}`;
    const raw = await kv.get(key);
    if (!raw) {
      return c.json({ memory: null, found: false });
    }
    const memory = JSON.parse(raw as string);
    console.log(`[TALK memory] Session memory loaded for ${userId}`);
    return c.json({ memory, found: true });
  } catch (err) {
    console.log(`[TALK memory] Error loading session memory: ${err}`);
    return c.json({ error: `TALK session memory load failed: ${err}` }, 500);
  }
});
