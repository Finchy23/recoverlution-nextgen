/**
 * INSIGHT SURFACE — The Anatomy of Truth (KNOW)
 *
 ...
 *
 * When the user enters KNOW from SYNC, they first see the insight
...
export function InsightSurface({ mode, breath, onResolve }: InsightSurfaceProps) {
  ...
  const handleComplete = useCallback((telemetry: SeekTelemetry) => {
    console.log('[KNOW] Arc complete — telemetry:', telemetry);
    sendTelemetry(telemetry);

    // ── KNOW → ∞MAP: Show K.B.E. bump confirmation ──
    setKbeConfirmation({
      knowing: telemetry.knowingScore,
      believing: telemetry.believingScore,
    });
    console.log(`[KNOW→∞MAP] K.B.E. bump: K=${telemetry.knowingScore.toFixed(2)} B=${telemetry.believingScore.toFixed(2)} for ${telemetry.insightId}`);
  ...
  return (
    <div ref={containerRef} className="absolute inset-0 overflow-hidden select-none">
      ...
      {/* ═══ K.B.E. BUMP CONFIRMATION — KNOW → ∞MAP ═══ */}
      <AnimatePresence>
        {kbeConfirmation && (
          <motion.div
            className="absolute pointer-events-none"
      ...
      {/* ═══ RESILIENCE WHISPER ═══ */}
      <ResilienceWhisper posture={resilience.posture} breath={breath} runtimeName="know" />
    </div>
  );
}