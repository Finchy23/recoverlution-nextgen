...
export function ActionSurface({ mode, breath, onResolve }: ActionSurfaceProps) {
  ...
  // ═══════════════════════════════════════════════════
  // HONE RECOMMENDATION (SYNC → HONE bridge)
  // ═══════════════════════════════════════════════════
  ...
  useEffect(() => {
    ...
    const timer = setTimeout(async () => {
      try {
        ...
        console.error('[SYNC→HONE] Recommendation failed:', err);
      }
    }, 2000);
      ...
      <AnimatePresence mode="wait">
        {showReceipt && (
          ...
          >
            ...
            {/* HONE recommendation — first scene only */}
            {recommendedPractice && scenesCompleted === 0 && (
              ...
              >
                ...
                >
                  HONE → {recommendedPractice.name.toUpperCase()}
                </span>
              </motion.div>
            )}
