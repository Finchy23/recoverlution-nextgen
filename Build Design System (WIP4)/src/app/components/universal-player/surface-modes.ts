 ...
 *
 * Architecture:
 *   HOME  → Default surface. The physics of feeling. Always home.
 ...
 *     SYNC → The sanctuary of understanding (forks → READ, KNOW, HONE)
 *     ECHO → The architecture of proof  (forks → PLOT, ∞MAP, LINK)
 *
 * HOME is not in the constellation — it IS the glass.
 * FLOW is not in the constellation — it lives in the Stream.
 * SYNC and ECHO are parent modes that expand a second arc.
 */
...
// ═══════════════════════════════════════════════════
// SYNC — The Sanctuary of Understanding (parent, forks)
// ═══════════════════════════════════════════════════

const syncParent: SurfaceMode = (() => {
  const r = recipeById('read'); // SYNC inherits READ's atmospheric DNA as its resting state
  return {
    id: 'sync',
    label: 'Sync',
...
const plot: SurfaceMode = (() => {
  ...
  return {
    ...
    temperatureRange: r.temperatureRange,
    sourceRooms: ['signal'],
    canopy: 'What do you notice today.',
    gesture: 'Observe.',
    copyDensity: 'light',
    arcPattern: 'descent-return',
    arrivalDelayMs: 0,
...
const map: SurfaceMode = (() => {
  ...
  return {
    ...
    magicLaw: r.magicLaw,
    streamFragments: [
      'Six territories of the brain',
      'The neural clusters are breathing',
    ...
    temperatureRange: r.temperatureRange,
    sourceRooms: ['map'],
    canopy: 'The landscape of your mind.',
    gesture: 'Navigate.',
    copyDensity: 'light',
    arcPattern: 'descent-return',
    arrivalDelayMs: 0,
