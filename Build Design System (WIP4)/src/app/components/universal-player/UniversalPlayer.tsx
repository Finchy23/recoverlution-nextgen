...
import { ActionSurface } from '../surfaces/ActionSurface';       // HOME
...
import { KnowSurface } from '../surfaces/KnowSurface';           // SYNC (parent — sanctuary entry)
import { ReadSurface } from '../surfaces/ReadSurface';           // READ (SYNC→)
import { SeekSurface } from '../surfaces/SeekSurface';           // KNOW (SYNC→ spatial documentary)
import { FormSurface } from '../surfaces/FormSurface';           // HONE (SYNC→)
...
function getSurfaceContent(
  ...
  switch (mode.id) {
    // The default surface — HOME is the glass
    case 'home':    return <ActionSurface mode={mode} breath={breath} onResolve={onResolve} />;
    ...
    // SYNC and its children
    case 'sync':    return <KnowSurface mode={mode} breath={breath} onResolve={onResolve} onNavigateToChild={onNavigateToChild} />;
    case 'read':    return <ReadSurface mode={mode} breath={breath} onResolve={onResolve} onNavigate={onNavigateToChild} />;
    case 'know':    return <SeekSurface mode={mode} breath={breath} onResolve={onResolve} />;
    case 'hone':    return <FormSurface mode={mode} breath={breath} onResolve={onResolve} />;
          ...
          overlay={({ breath, atmosphere, isPhone }) => (
            <>
              ...
              <PlayerAnchor
                ...
                onTap={() => {
                  if (!inCueFlow) {
                    // In a surface — return home
                    handleResolve();
                  }
                  // At home — the orb acknowledges the touch but does nothing.
                  // Stream is swipe-only. Constellation is hold-only.
                }}
