...
Add one frontend session provider or preview persona context so person-bound rooms do not invent their own identity assumptions.

### 4. Event seam
Add one event adapter so room events can later flow into PostHog without retrofitting every component.

### 5. Media seam
Add one adapter layer for:
- JW Player video
- audio transport / Tone.js-ready playback shell
- fallback media handling

### 6. Resilience seam
Add a single fallback / degraded-mode seam for:
- network loss
- missing payload
...
- `src/app/components/runtime/useJWMedia.ts`
- `src/app/components/runtime/usePlayRuntime.ts`
- `src/app/components/runtime/useFormRuntime.ts`
- `src/app/components/talk/talk-runtime.ts`

## Known steering notes

### 1. Talk seam
The current Talk client is pointed at an old function seam and should eventually be normalized to the reviewed `talk-runtime` path.
Do not invent a local second Talk backend to compensate.

### 2. Play seam
The current Play hook is directionally right, but its comments and assumption framing should eventually be aligned to the real reviewed `play-runtime` contract.
